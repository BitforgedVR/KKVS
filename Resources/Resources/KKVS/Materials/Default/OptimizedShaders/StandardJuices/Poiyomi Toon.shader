Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Toon/0348f1c60e13d15adaf92a67cabf5499"
{
    Properties
    {
        ...
        [HideInInspector] m_start_uvDistortion (" Distortion UV--{reference_property:_EnableDistortion, button_help:{text:Tutorial,action:{type:URL,data:https://www.poiyomi.com/modifiers/uvs/distortion-uv},hover:Documentation}}", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            cbuffer LightVolumeUniforms
            {
                ...
                // Legacy! Used in this version to have back compatibility with older worlds. Array commented above will be used in future releases! Legacy!
                ...
                // Legacy! AABB Bounds of islands on the 3D Texture atlas. Array commented above will be used in future releases! Legacy!
            ...
            void LV_QuadLight(float3 worldPos, float3 centroidPos, float4 rotationQuat, float2 size, float3 color,
                                                                 float sqMaxDist, float occlusion, inout float3 L0,
                                                                 inout float3 L1r, inout float3 L1g, inout float3 L1b,
                                                                 inout uint count)
            {
            ...
            float3 LV_PointLightAttenuation(float sqdist, float sqlightSize, float3 color, float brightnessCutoff,
                float sqMaxDist)
            {
            ...
            void LV_SphereLight(float sqdist, float3 dirN, float sqlightSize, float3 color, float occlusion,
                     float sqMaxDist, inout float3 L0, inout float3 L1r, inout float3 L1g, inout float3 L1b,
                     inout uint count)
            {
                float3 att = LV_PointLightAttenuation(sqdist, sqlightSize, color, _UdonLightBrightnessCutoff,
                                       sqMaxDist);
            ...
            void LV_SphereSpotLight(float sqdist, float3 dirN, float sqlightSize, float3 att, float spotMask,
                                  float cosAngle, float coneFalloff, float occlusion, inout float3 L0, inout float3 L1r,
                                  inout float3 L1g, inout float3 L1b, inout uint count)
            {
            ...
            void LV_SphereSpotLightCookie(float sqdist, float3 dirN, float sqlightSize, float3 att, float4 lightRot,
            float tanAngle, uint customId, float occlusion, inout float3 L0, inout float3 L1r, inout float3 L1g,
            inout float3 L1b, inout uint count)
            {
            ...
            void LV_SphereSpotLightAttenuationLUT(float sqdist, float3 dirN, float sqlightSize, float3 color,
                        float spotMask, float cosAngle, uint customId, float occlusion,
                        inout float3 L0, inout float3 L1r, inout float3 L1g,
                        inout float3 L1b, inout uint count)
            {
            ...
            void LV_PointLight(uint id, float3 worldPos, float4 occlusion, inout float3 L0, inout float3 L1r,
                                                              inout float3 L1g, inout float3 L1b, inout uint count)
            {
                ...
                if (pos.w < 0)
                {
                    ...
                    if (customId > 0)
                    {
                        // If it uses Attenuation LUT
                        LV_SphereSpotLightAttenuationLUT(sqlen, dirN, -pos.w, color.rgb, spotMask, angle, customId,
lightOcclusion, L0, L1r, L1g, L1b, count);
                    }
                    else
                    {
                        // If it uses default parametric attenuation
                        float3 att = LV_PointLightAttenuation(sqlen, -pos.w, color.rgb, _UdonLightBrightnessCutoff,
                     sqrRange);
                        if (customId < 0)
                        {
                            // If uses cookie
                            LV_SphereSpotLightCookie(sqlen, dirN, -pos.w, att, ldir, angle, customId, lightOcclusion,
                            L0, L1r, L1g, L1b, count);
                        }
                        else
                        {
                            // If it uses default parametric attenuation
                            LV_SphereSpotLight(sqlen, dirN, -pos.w, att, spotMask, angle, ldir.w, lightOcclusion, L0,
                                                    L1r, L1g, L1b, count);
                        }
                    }
                }
                else if (color.w <= 1.5f)
                {
                    ...
                    if (customId > 0)
                    {
                        ...
                        // Sign of range defines if it's point light (positive) or a spot light (negative)
                    ...
                    else
                    {
                        ...
                        if (customId < 0)
                        {
                            // If it uses a cubemap
                            uint id = -customId - 1;
                            // Cubemap ID starts from zero and should not take in count texture array slices count.
                ...
                {
                    // It is an area light
                    LV_QuadLight(worldPos, pos.xyz, ldir, float2(pos.w, color.w - 2.0f), color.rgb, sqrRange,
                                                                        lightOcclusion, L0, L1r, L1g, L1b, count);
                }
            }
            ...
            void LV_SampleVolume(uint id, float3 localUVW, inout float3 L0, inout float3 L1r, inout float3 L1g,
                inout float3 L1b, out float4 occlusion)
            {
                uint uvwID = id * 6;
                float3 uvwScaled = saturate(localUVW + 0.5) * (_UdonLightVolumeUvw[uvwID + 1].xyz - _UdonLightVolumeUvw[
                    uvwID].xyz);
            ...
            float4 LV_SampleVolumeOcclusion(uint id, float3 localUVW)
            {
                ...
                {
                    uint uvwID = id * 6;
                    float3 uvwScaled = saturate(localUVW + 0.5) * (_UdonLightVolumeUvw[uvwID + 1].xyz -
                        _UdonLightVolumeUvw[uvwID].xyz);
            ...
            void LV_PointLightVolumeSH(float3 worldPos, float4 occlusion, inout float3 L0, inout float3 L1r,
                                                                   inout float3 L1g, inout float3 L1b)
            {
            ...
            void LV_LightVolumeSH(float3 worldPos, inout float3 L0, inout float3 L1r, inout float3 L1g,
                           inout float3 L1b, out float4 occlusion)
            {
                occlusion = 1;
                ...
                if (volumesCount == 0)
                {
                    // Legacy! Fallback to default light probes if Light Volume are not enabled or a version is too old to have a support. Legacy!
            ...
            void LV_LightVolumeAdditiveSH(float3 worldPos, inout float3 L0, inout float3 L1r, inout float3 L1g,
                                                         inout float3 L1b, out float4 occlusion)
            {
                occlusion = 1;
                float4 mOcclusion = 1; // Multiplicative occlusion. Applies on top of regular occlusion
                ...
                bool isNoB = true;
                uint addVolumesCount = 0;
                uint count = min(_UdonLightVolumeOcclusionCount == 0 ? additiveCount : volumesCount,
                    VRCLV_MAX_VOLUMES_COUNT); // Only use all volumes if occlusion volumes are enabled
            ...
            float3 LightVolumeSpecular(float3 f0, float smoothness, float3 worldNormal, float3 viewDir, float3 L0,
                        float3 L1r, float3 L1g, float3 L1b)
            {
                float3 specColor = max(float3(dot(reflect(-L1r, worldNormal), viewDir),
                                 dot(reflect(-L1g, worldNormal), viewDir), dot(reflect(-L1b, worldNormal), viewDir)),
                          0);
            ...
            float3 LightVolumeSpecular(float3 albedo, float smoothness, float metallic, float3 worldNormal,
                         float3 viewDir, float3 L0, float3 L1r, float3 L1g, float3 L1b)
            {
            ...
            float3 LightVolumeSpecularDominant(float3 albedo, float smoothness, float metallic, float3 worldNormal,
                         float3 viewDir, float3 L0, float3 L1r, float3 L1g, float3 L1b)
            {
                float3 specularf0 = lerp(0.04f, albedo, metallic);
            ...
            data.boxMin[1], data.boxMax[1]);
                #endif
                half3 env1 = Unity_GlossyEnvironment(UNITY_PASS_TEXCUBE_SAMPLER(unity_SpecCube1, unity_SpecCube0),
                                                                data.probeHDR[1], glossIn);
...
blendLighten(base.b, blend.b));
            ...
            VertexOut vert(appdata v)
            {
                ...
                vertexAudioLink[0] = 0.0 == 0
                              ? AudioLinkData(
                                  ALPASS_AUDIOLINK + float2(0, 0))[0]
                              : AudioLinkData(
                                  ALPASS_FILTEREDAUDIOLINK + float2(
                                      (1 - 0.0) * 15.95, 0))[0];
                vertexAudioLink[1] = 0.0 == 0
            ? AudioLinkData(
                ALPASS_AUDIOLINK + float2(0, 1))[0]
            : AudioLinkData(
                ALPASS_FILTEREDAUDIOLINK + float2(
                    (1 - 0.0) * 15.95, 1))[0];
                ...
                in PoiMods poiMods)
            {
                ...
                if (1.0 > 0)
                {
                    poiFragData.alpha = maskBlend(poiFragData.alpha, poiMods.globalMask[1.0 - 1], 2.0);
                }
            }
            ...
            {
                #if defined(PROP_GLOBALMASKTEXTURE0) || !defined(OPTIMIZER_ENABLED)
                float4 gmcol0 = POI2D_SAMPLER_PAN(_GlobalMaskTexture0, _MainTex,
                                              poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
                if (0)
                {
                    poiMods.globalMask[0] = gmcol0.r;
                    poiMods.globalMask[1] = POI2D_SAMPLER_PAN(_GlobalMaskTexture0, _MainTex,
                        poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).g;
                    poiMods.globalMask[2] = POI2D_SAMPLER_PAN(_GlobalMaskTexture0, _MainTex,
                                       poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).b;
                    poiMods.globalMask[3] = POI2D_SAMPLER_PAN(_GlobalMaskTexture0, _MainTex,
                                                     poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).a;
                }
                else
                {
                ...
                #endif
                #if defined(PROP_GLOBALMASKTEXTURE1) || !defined(OPTIMIZER_ENABLED)
                float4 gmcol1 = POI2D_SAMPLER_PAN(_GlobalMaskTexture1, _MainTex,
                poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0));
                if (0)
                {
                    poiMods.globalMask[4] = gmcol1.r;
                    poiMods.globalMask[5] = POI2D_SAMPLER_PAN(_GlobalMaskTexture1, _MainTex,
                             poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).g;
                    poiMods.globalMask[6] = POI2D_SAMPLER_PAN(_GlobalMaskTexture1, _MainTex,
                    poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).b;
                    poiMods.globalMask[7] = POI2D_SAMPLER_PAN(_GlobalMaskTexture1, _MainTex,
            ...
            float4(0, 0, 0, 0)).a;
                ...
                #endif
                #if defined(PROP_GLOBALMASKTEXTURE2) || !defined(OPTIMIZER_ENABLED)
                float4 gmcol2 = POI2D_SAMPLER_PAN(_GlobalMaskTexture2, _MainTex,
                    poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0));
                if (0)
                {
                    poiMods.globalMask[8] = gmcol2.r;
                    poiMods.globalMask[9] = POI2D_SAMPLER_PAN(_GlobalMaskTexture2, _MainTex,
                    poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).g;
                    poiMods.globalMask[10] = POI2D_SAMPLER_PAN(_GlobalMaskTexture2, _MainTex,
                         poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).b;
                    poiMods.globalMask[11] = POI2D_SAMPLER_PAN(_GlobalMaskTexture2, _MainTex,
                poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).a;
                }
                else
                {
                ...
                #endif
                #if defined(PROP_GLOBALMASKTEXTURE3) || !defined(OPTIMIZER_ENABLED)
                float4 gmcol3 = POI2D_SAMPLER_PAN(_GlobalMaskTexture3, _MainTex,
                                                              poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)),
                                                              float4(0, 0, 0, 0));
                if (0)
                {
                    poiMods.globalMask[12] = gmcol3.r;
                    poiMods.globalMask[13] = POI2D_SAMPLER_PAN(_GlobalMaskTexture3, _MainTex,
                        poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).g;
                    poiMods.globalMask[14] = POI2D_SAMPLER_PAN(_GlobalMaskTexture3, _MainTex,
                                        poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).b;
                    poiMods.globalMask[15] = POI2D_SAMPLER_PAN(_GlobalMaskTexture3, _MainTex,
                              poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)).a;
                }
                else
                {
            ...
            {
                #if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
                float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex,
                    poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0.2,0.3,0,0)) * 2 - 1;
                #else
                float4 flowVector = -1;
                #endif
                #if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
                float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex,
                                                                poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)),
                                                                float4(0, 0, 0, 0)) * 2 - 1;
                #else
                ...
                #endif
                #if defined(PROP_DISTORTIONMASK) || !defined(OPTIMIZER_ENABLED)
                half distortionMask = POI2D_SAMPLER_PAN(_DistortionMask, _MainTex, poiMesh.uv[0.0], float4(0, 0, 0, 0))[
                    0.0];
                #else
                half distortionMask = 1;
                #endif
                half distortionStrength = 0.08;
                half distortionStrength1 = 0.0;
                #ifdef POI_AUDIOLINK

                if (AudioLinkIsAvailable() && 0.0 && 1.0)
                {
                    distortionStrength += lerp(float4(0, 0, 0, 0).x, float4(0, 0, 0, 0).y,
                       AudioLinkData(uint2(0, uint(0.0))).r);
                    distortionStrength1 += lerp(float4(0, 0, 0, 0).x, float4(0, 0, 0, 0).y,
                        AudioLinkData(uint2(0, uint(0.0))).r);
                }
                #endif
    ...
    half fresnelTerm)
                                       ...
                                       in PoiCam poiCam)
            {
                ...
                poiLight.finalLighting += clamp(LightVolumeEvaluate(poiMesh.normals[1], L0, L1r, L1g, L1b),
                    _LightingMinLightBrightness, _LightingCap);
                #endif
				}
                #endif
                #endif
                if (poiFragData.toggleVertexLights)
                {
                    ...
                    if (lightingMode == 1)
                    {
                        ...
                        if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
                        vertexLighting = max(vertexLighting,
                                             lerp(poiLight.vColor[index], poiLight.vColor[index] * 1.0,
                              ...
                              in PoiMods poiMods)
            {
                ...
                #if defined(PROP_CUBEMAP) || !defined(OPTIMIZER_ENABLED)
                if (any(float4(0, 0, 0, 0).xyz) || any(float4(0, 3, 0, 0).xyz))
                {
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.yxz,
                                                       float4(0, 0, 0, 0).x + (float4(0, 3, 0, 0).x * _Time.y)).yxz;
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xyz,
                                                       float4(0, 0, 0, 0).y + (float4(0, 3, 0, 0).y * _Time.y)).xyz;
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xzy,
                                                       float4(0, 0, 0, 0).z + (float4(0, 3, 0, 0).z * _Time.y)).xzy;
                }
                float mipLevel = (1 - 0.218) * (1 - 0.218) * 8;
                float4 cubeMap = _CubeMap.SampleLevel(sampler_linear_clamp, CubeMapUV, mipLevel);
                cubeMap.rgb *= poiThemeColor(poiMods, float4(0.280335, 0.2520297, 0, 1), 0.0);
                #else
                float4 cubeMap = float4(0.21763764082, 0.21763764082, 0.21763764082, .5) * float4(
                    poiThemeColor(poiMods, float4(0.280335, 0.2520297, 0, 1), 0.0), 1);
                #endif
                cubeMap.rgb *= 0.77;
                #if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
                float CubeMapMask = POI2D_SAMPLER_PAN(_CubeMapMask, _MainTex,
                                                      poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)),
                                                      float4(0, 0, 0, 0))[0.0];
                #else
                float CubeMapMask = 1;
                ...
                if (1.0 > 0)
                {
                    CubeMapMask = maskBlend(CubeMapMask, poiMods.globalMask[1.0 - 1], 2.0);
                }
                if (0.0)
                {
                ...
                if (0.0)
                {
                    ...
                    cubeMap = PoiColorBCS(cubeMap, 0.0, 1.0, 3.19);
                }
                CubeMapMask = min(CubeMapMask, lerp(1, poiLight.rampedLightMap, 0.099));
            ...
            float3 GetReflections(in PoiCam poiCam, in PoiLight pl, in PoiMesh poiMesh, float perceptualRoughness,
                                  float ForceFallback, float LightFallback, TextureCube reflectionCube, float4 hdrData,
                                  float3 reflectionDir)
            {
            ...
            float SFVisibility(float brdfRoughness, float3 directColor, float NDotV, float ExposureOcclusion)
            {
                ...
                //Using direct color because I think it should be generally more forgiving
            ...
            void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
            {
                ...
                if (0.0)
                {
                    ...
                    assignValueToVectorFromIndex(PBRMaps, 0.0, PBRSplitMask[0.0]);
                            ...
                            in PoiMesh poiMesh, inout PoiMods poiMods)
            {
                ...
                specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(
                    poiMods, float4(0.218987, 0.2541522, 0, 1), 0.0) * poiLight.occlusion * attenuation;
                if (poiFragData.toggleVertexLights)
                {
                    #if defined(VERTEXLIGHT_ON)
                    for (int index = 0; index < 4; index++)
                    {
                        ...
                        vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(
                            poiMods, float4(0.218987, 0.2541522, 0, 1), 0.0) * poiLight.occlusion;
                    }
                    #endif
                }
                if (0.0 == 1)
                {
                    ...
                    specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(
                        poiMods, float4(0.218987, 0.2541522, 0, 1), 0.0) * poiLight.occlusion * attenuation * 1.0;
                    if (poiFragData.toggleVertexLights)
                    {
                        #if defined(VERTEXLIGHT_ON)
                        for (int index = 0; index < 4; index++)
                        {
                            ...
                            vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask *
                                poiThemeColor(poiMods, float4(0.218987, 0.2541522, 0, 1), 0.0) * poiLight.occlusion *
                                1.0;
                        }
                        #endif
                    }
                ...
                float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, percepRough, 1.0, 1.0, _MochieReflCube,
                ...
                reflections = surfaceReduction * reflCol * FresnelLerp(specCol, specCol + lerp(specCol, 1, 0.5) * 0.5,
                                                                       pbrNDotV) * SFVisibility(
                    brdfRoughness, poiLight.directColor, pbrNDotV, 0.0);
                reflections *= poiThemeColor(poiMods, float4(0.244029, 0.2746773, 0, 1), 0.0);
            ...
            float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
            {
                ...
                poiMesh.uv[9] = float2(dot(worldViewRight, poiMesh.normals[0]),
                                       dot(worldViewUp, poiMesh.normals[0])) * 0.5 + 0.5;
                #ifdef USER_LUT
                ...
                #if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
                poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(
                    _BumpMap, _MainTex, poiUV(poiMesh.uv[7.0].xy, float4(5,5,-1.42,0)), float4(0,0,0,0), 0.0), 1.71);
                #else
                ...
                if (0.0 == 0)
                {
                    poiLight.direction = calculateluminance(_LightColor0.rgb) * _WorldSpaceLightPos0.xyz + (PoiSHAr.xyz
                        + PoiSHAg.xyz + PoiSHAb.xyz) / 3.0;
                }
                if (0.0 == 1 || 0.0 == 2)
                {
                    if (0.0 == 1)
                ...
                if (0.0 > 0)
                {
                    ...
                    lightDataSDFMap = POI2D_SAMPLER_PANGRAD(_LightDataSDFMap, _linear_repeat,
                                                            poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)),
                                                            float4(0, 0, 0, 0), max(poiMesh.dx, sdfLod),
                                                            max(poiMesh.dy, sdfLod)).rg;
                }
                else
                {
                ...
                else
                {
                    poiLight.directColor = max(poiLight.directColor,
                                               poiLight.directColor * min(
                                                   10000, (_LightingMinLightBrightness * rcp(
                ...
                poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.3119799, 0.2036198, 0.006655933, 1).rgb, 0.0);
                poiFragData.alpha = mainTexture.a * float4(0.3119799, 0.2036198, 0.006655933, 1).a;
                #else
...
poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).g;
            ...
            float2 distortedUV(in PoiMesh poiMesh)
            {
                ...
                #if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
                float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex,
                            poiUV(poiMesh.uv[0.0], float4(1, 1, 0, 0)), float4(0, 0, 0, 0)) * 2 - 1;
                #else
                float4 flowVector1 = -1;
                #endif
                #if defined(PROP_DISTORTIONMASK) || !defined(OPTIMIZER_ENABLED)
...
1 - (.5 * poiLight.nDotL + .5));
                                                                    ...
                                                                    in PoiMods poiMods)
            {
                ...
                if (any(float4(0, 0, 0, 0).xyz) || any(float4(0, 3, 0, 0).xyz))
                {
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.yxz,
                                                    float4(0, 0, 0, 0).x + (float4(0, 3, 0, 0).x * _Time.y)).yxz;
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xyz,
                            float4(0, 0, 0, 0).y + (float4(0, 3, 0, 0).y * _Time.y)).
                        xyz;
                    CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xzy,
                        float4(0, 0, 0, 0).z + (float4(0, 3, 0, 0).z * _Time.y)).xzy;
                }
                ...
                #if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
                float CubeMapMask = POI2D_SAMPLER_PAN(_CubeMapMask, _MainTex,
                              poiUV(poiMesh.uv[0.0],
                                                        float4(1, 1, 0, 0)), float4(0, 0, 0, 0))[0.0];
                #else
                float CubeMapMask = 1;
                #endif
                if (1.0 > 0)
            ...
            float3 GetReflections(in PoiCam poiCam, in PoiLight pl, in PoiMesh poiMesh, float perceptualRoughness,
                             float ForceFallback, float LightFallback, TextureCube reflectionCube,
                             float4 hdrData, float3 reflectionDir)
            {
                float3 reflections = 0;
                float3 lighting = pl.finalLighting;
                if (ForceFallback == 0)
