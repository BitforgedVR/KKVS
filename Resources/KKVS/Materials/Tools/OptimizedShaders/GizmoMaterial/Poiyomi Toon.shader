Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/133ac0033ffe4335eb49a899fc741363"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void LV_QuadLight(float3 worldPos, float3 centroidPos, float4 rotationQuat, float2 size, float3 color,
                float sqMaxDist, float occlusion, inout float3 L0, inout float3 L1r, inout float3 L1g, inout float3 L1b,
                inout uint count)
            {
        ...
        float sqMaxDist)
            ...
            void LV_SphereSpotLight(float sqdist, float3 dirN, float sqlightSize, float3 att, float spotMask,
                                              float cosAngle, float coneFalloff, float occlusion,
                                              inout float3 L0, inout float3 L1r,
            ...
            void LV_SphereSpotLightAttenuationLUT(float sqdist, float3 dirN, float sqlightSize, float3 color,
                            ...
                            float occlusion, inout float3 L0, inout float3 L1r,
...
inout float3 L1b, out float4 occlusion)
            ...
            float3 LightVolumeSpecularDominant(float3 albedo, float smoothness, float metallic, float3 worldNormal,
                                                                    float3 viewDir, float3 L0, float3 L1r, float3 L1g,
                                                                    float3 L1b)
            {
                float3 specularf0 = lerp(0.04f, albedo, metallic);
...
data.boxMin[1], data.boxMax[1]);
            ...
            float3 randomFloat3WiggleRange(float2 Seed, float Range, float wiggleSpeed, float timeOffset)
            {
                ...
                return float3(sin(((_Time.x + timeOffset) + rando.x * PI) * speed),
                                                                           sin(((_Time.x + timeOffset) + rando.y * PI) *
                                                                               speed),
                                                                           sin(((_Time.x + timeOffset) + rando.z * PI) *
                                                                               speed)) * Range;
            }
...
in PoiMods poiMods)
            ...
            void applyVertexColor(inout PoiFragData poiFragData, PoiMesh poiMesh)
            {
                if (1.0)
                {
                    ...
                    float3 vertCol = lerp(poiMesh.vertexColor.rgb, GammaToLinearSpace(poiMesh.vertexColor.rgb), 1.0);
                    poiFragData.baseColor *= lerp(1, vertCol, 1.0);
                    #endif
                    poiFragData.alpha *= lerp(1, poiMesh.vertexColor.a, 0.0);
                }
            }
            ...
            float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
            {
                ...
                poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.8000157, 0.7198833, 0.7642724, 1).rgb, 0.0);
                poiFragData.alpha = mainTexture.a * float4(0.8000157, 0.7198833, 0.7642724, 1).a;
                #else
                ...
                {
                    poiFragData.baseColor *=
                        poiThemeColor(poiMods, float4(0.8000157, 0.7198833, 0.7642724, 1).rgb, 0.0);
                    poiFragData.alpha = mainTexture.a * float4(0.8000157, 0.7198833, 0.7642724, 1).a;
                }
                #endif
                if (2.0)
