Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/0348f1c60e13d15adaf92a67cabf5499"
{
	Properties
	{
		...
		[HideInInspector][Vector2]_DistortionFlowTexturePan ("Panning", Vector) = (0, 2, 0, 0)
		...
		[HideInInspector][Vector2]_DistortionFlowTexture1Pan ("Panning", Vector) = (2, 0, 0, 0)
		...
		_DistortionStrength ("Strength1", Float) = 0.05
		_DistortionStrength1 ("Strength2", Float) = 0.02
 ...
 #define OPTIMIZER_ENABLED 
			...
			VertexOut vert(appdata v)
			{
				...
				float basicsMask = vertexEffectsMask[1.0];
				...
				if (_VertexWindEnabled)
				{
					...
					if (windMask > 0)
					{
						...
						#if defined(PROP_VERTEXBASICSMASK) || !defined(OPTIMIZER_ENABLED)
						float noise = _VertexBasicsMask.SampleLevel(sampler_linear_repeat, poiUV(noiseUV, float4(1,1,0,0)), 0)[3.0] * 2 - 1;
						#else
						float noise = 0;
						#endif
						float turbulence = lerp(1, noise, 0.0);
						float primaryWave = sin(_Time.y * 0.5 + dot(worldPos, normalize(float4(1,0,0,0).xyz)) * 1.0) * turbulence;
						float detailWave = sin(_Time.y * 0.5 + dot(worldPos, normalize(float4(0,-1,0,0).xyz)) * 1.0) * turbulence;
						float3 primaryOffset = primaryWave * normalize(float4(1,0,0,0).xyz) * 0.09;
						float3 detailOffset = detailWave * normalize(float4(0,-1,0,0).xyz) * 0.03;
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + -0.094);
				if (1.0 > 0)
				{
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0.2,0.3,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector1 = POI2D_SAMPLER_PAN(_DistortionFlowTexture1, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)) * 2 - 1;
				#else
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_DetailTex, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0,0)), float4(0.004,0.004,0,0), 0.0).rgb * poiThemeColor(poiMods, float4(1,1,1,1), 0.0);
				#else
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[7.0], float4(1,1,0.94,0)), float4(0,0,0,0), 1.0), 3.06 * poiMods.detailMask.g);
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2(0.0, 0.5);
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * 1.0, smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.yxz, float4(0,0,0,0).x + (float4(0,3,0,0).x * _Time.y)).yxz;
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xyz, float4(0,0,0,0).y + (float4(0,3,0,0).y * _Time.y)).xyz;
					CubeMapUV = RotateAroundYInDegrees(CubeMapUV.xzy, float4(0,0,0,0).z + (float4(0,3,0,0).z * _Time.y)).xzy;
				}
				float mipLevel = (1 - 0.165) * (1 - 0.165) * 8;
				float4 cubeMap = _CubeMap.SampleLevel(sampler_linear_clamp, CubeMapUV, mipLevel);
				cubeMap.rgb *= poiThemeColor(poiMods, float4(0.04539379,0.03723783,0.00146381,1), 0.0);
				#else
				float4 cubeMap = float4(0.21763764082, 0.21763764082, 0.21763764082, .5) * float4(poiThemeColor(poiMods, float4(0.04539379,0.03723783,0.00146381,1), 0.0), 1);
				#endif
				cubeMap.rgb *= 1.04;
				#if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				poiFragData.emission += cubeMap.rgb * 0.68 * CubeMapMask * cubeMap.a;
			}
			#endif
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.218987,0.2541522,0,1), 0.0) * poiLight.occlusion * attenuation;
				if (poiFragData.toggleVertexLights)
				{
					#if defined(VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.218987,0.2541522,0,1), 0.0) * poiLight.occlusion;
					}
					#endif
				}
				if (0.0 == 1)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.218987,0.2541522,0,1), 0.0) * poiLight.occlusion * attenuation * 1.0;
					if (poiFragData.toggleVertexLights)
					{
						#if defined(VERTEXLIGHT_ON)
						for (int index = 0; index < 4; index++)
						{
							...
							vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.218987,0.2541522,0,1), 0.0) * poiLight.occlusion * 1.0;
						}
						#endif
					}
				}
				...
				reflections *= poiThemeColor(poiMods, float4(0.3261475,0.3691636,0.001171048,1), 0.0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[7.0].xy, float4(5,5,-1.42,0)), float4(0,0,0,0), 0.0), 1.0);
				#else
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.3812781,0.3404159,0.006052644,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.3812781,0.3404159,0.006052644,1).a;
				#else
