Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/e77dc9a18c5a9f4178899e89cd1ce29e"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void LV_SphereLight(float sqdist, float3 dirN, float sqlightSize, float3 color, float occlusion,
                                               float sqMaxDist, inout float3 L0, inout float3 L1r,
            ...
            void LV_SphereSpotLight(float sqdist, float3 dirN, float sqlightSize, float3 att, float spotMask,
                                        float cosAngle, float coneFalloff, float occlusion, inout float3 L0,
...
data.boxMin[0], data.boxMax[0]);
    ...
    blendColorBurn(base.b, blend.b));
        ...
        : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - 0.0) * 15.95, 2))[0];
                ...
                if (0.0 == 0) // Discard Vertices instead of just pixels
                {
                    ...
                    UDIMDiscardRows[3] = float4(_UDIMDiscardRow3_0, _UDIMDiscardRow3_1, _UDIMDiscardRow3_2,
                      _UDIMDiscardRow3_3);
    ...
    : max((1.0 - ((1.0 - base) * rcp(random_uniform_float_only_used_to_stop_compiler_warnings + blend))),
                           ...
                           in PoiCam poiCam)
            {
                ...
                if (poiFragData.toggleVertexLights)
                {
                    ...
                    if (lightingMode == 0)
                    {
                        vertexLighting = max(vertexLighting,
                                                                        ...
                                                                        vSaturatedDotNL[index] * poiLight.detailShadow);
                        // Realistic
                    }
                    if (lightingMode == 1)
                    {
                ...
                lerp(poiLight.vColor[index],
                       poiLight.vColor[index] * 0.5,
                       smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) *
                poiLight.
                detailShadow);
                    }
					}
            ...
            struct PoiDecal
            {
                     ...
                     in PoiCam poiCam, float2 scaleMultiplier = float2(1, 1))
                {
                    ...
                    decalColor = tex2D(decalTexture, poiUV(uv, m_DecalTexture_ST) + m_DecalTexturePan * _Time.x,
                        ...
                        m_DecalColor.a);
                    decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb,
                        m_DecalHueShift + poiLight.nDotVCentered * m_DecalHueAngleStrength, m_DecalHueShiftSpeed,
                        m_DecalHueShiftColorSpace, m_DecalHueShiftSelectOrShift);
                    decalColor.a *= decalMask[m_DecalMaskChannel] * applyTilingClipping(m_DecalTiled, uv);
                }
                ...
                : float4(ddx(uvSample0) * m_DecalTexture_ST.x, ddy(uvSample0) * m_DecalTexture_ST.y);
                    float4 dduvSample1 = any(fwidth(uvSample1) > .5)
                                                                              ? 0.001
                                                                              : float4(
                                                                                  ddx(uvSample1) * m_DecalTexture_ST.x,
                                                                                  ddy(uvSample1) * m_DecalTexture_ST.y);
                    float4 sample0 = tex2D(decalTexture,
