Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/0acd5f5133cebfccb8e740816bc7bb78"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void LV_QuadLight(float3 worldPos, float3 centroidPos, float4 rotationQuat, float2 size, float3 color,
                     float sqMaxDist, float occlusion, inout float3 L0, inout float3 L1r, inout float3 L1g,
                     inout float3 L1b,
                     inout uint count)
            {
            ...
            void LV_SphereSpotLightCookie(float sqdist, float3 dirN, float sqlightSize, float3 att, float4 lightRot,
                                                           float tanAngle, uint customId, float occlusion,
            ...
            void LV_SphereSpotLightAttenuationLUT(float sqdist, float3 dirN, float sqlightSize, float3 color,
                                                                  float spotMask, float cosAngle, uint customId,
            ...
            dot(reflect(-L1g, worldNormal), viewDir), dot(reflect(-L1b, worldNormal), viewDir)), 0);
    ...
    0, 0, 0, 1);
            ...
            VertexOut vert(appdata v)
            {
                ...
                if (0.0 == 0) // Discard Vertices instead of just pixels
                {
                    ...
                    UDIMDiscardRows[0] = float4(_UDIMDiscardRow0_0, _UDIMDiscardRow0_1, _UDIMDiscardRow0_2,
                                       _UDIMDiscardRow0_3);
                ...
                if (poiFragData.toggleVertexLights)
                {
                    ...
                    if (lightingMode == 0)
                    {
                        vertexLighting = max(vertexLighting,
                          poiLight.vColor[index] * poiLight.
                          vSaturatedDotNL[index] * poiLight.
        ...
        in PoiMesh poiMesh, in PoiCam poiCam)
                             ...
                             in PoiLight poiLight)
            {
                ...
                Decal0.m_DecalScale = float4(0.65, 0.55, 1, 0);
                Decal0.m_DecalSideOffset = float4(0, 0, 0, 0);
                Decal0.m_DecalPosition = float4(0.32, 0.27, 0, 0);
                ...
                Decal0.m_DecalOverrideAlpha = 2.0;
...
blendLighten(base.b, blend.b));
    ...
    themeColorExposures.z), float4(1, 1, 1, 1).a);
                                                                  ...
                                                                  in PoiCam poiCam)
            {
                ...
                if (poiFragData.toggleVertexLights)
                {
                    ...
                    {
                        vertexLighting = max(vertexLighting,
                                                                   poiLight.vColor[index] * poiLight.vSaturatedDotNL[
                                                                       index] * poiLight.detailShadow); // Realistic
                    }
                    if (lightingMode == 1)
                    {
                        float2 ToonAddGradient = float2(0.0, 0.5);
    ...
    1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.
                                                 ...
                                                 in PoiMesh poiMesh, in PoiCam poiCam)
            {
                ...
                float sn = sin(theta);
                uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x,
                                                                      (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.
                                                                          y) * cs + decalCenter.y);
                uv = remap(uv, float2(0, 0) - scale / 2 + position + scaleOffset.xz,
        ...
        poiMods.audioLink[m_AudioLinkDecalScaleBand]);
                      ...
                      poiMods.audioLink[m_AudioLinkDecalSideBand]);
                        ...
                        decalChannelOffset += lerp(m_AudioLinkDecalChannelSeparation[0],
                             m_AudioLinkDecalChannelSeparation[1],
                             poiMods.audioLink[
                                 m_AudioLinkDecalChannelSeparationBand]);
                    }
                    #endif
                }
...
m_DecalSymmetryMode, m_DecalMirroredUVMode, poiMesh, poiCam);
                    ...
                    decalColor = tex2D(decalTexture, poiUV(uv, m_DecalTexture_ST) + m_DecalTexturePan * _Time.x,
        dduv.xy, dduv.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
                    decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb,
                        m_DecalHueShift + poiLight.nDotVCentered * m_DecalHueAngleStrength,
    ...
    : float4(ddx(uv) * m_DecalTexture_ST.x,
            ...
            : float4(ddx(uvSample1) * m_DecalTexture_ST.x,
                    ...
                    float4 sample0 = tex2D(decalTexture,
                        ...
                        poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
                    float4 sample1 = tex2D(decalTexture,
                        poiUV(uvSample1, m_DecalTexture_ST) +
                        m_DecalTexturePan * _Time.x, dduvSample1.xy,
