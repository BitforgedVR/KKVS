Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/573c0944ff2ec79e3bb31458a7c25d8d"
{
    Properties
    {
        [HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.3.63</color>", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void LV_QuadLight(float3 worldPos, float3 centroidPos, float4 rotationQuat, float2 size, float3 color,
                     float sqMaxDist, float occlusion, inout float3 L0, inout float3 L1r,
            ...
            void LV_SphereLight(float sqdist, float3 dirN, float sqlightSize, float3 color, float occlusion,
                                               float sqMaxDist, inout float3 L0, inout float3 L1r, inout float3 L1g,
                                               inout float3 L1b,
                                               inout uint count)
            {
            ...
            void LV_SphereSpotLight(float sqdist, float3 dirN, float sqlightSize, float3 att, float spotMask,
                                                                        float cosAngle, float coneFalloff,
                                                                        float occlusion, inout float3 L0,
                                                                        inout float3 L1r, inout float3 L1g,
            ...
            void LV_SphereSpotLightCookie(float sqdist, float3 dirN, float sqlightSize, float3 att, float4 lightRot,
                                   float tanAngle, uint customId, float occlusion, inout float3 L0, inout float3 L1r,
                                   inout float3 L1g,
                                   inout float3 L1b, inout uint count)
            {
            ...
            void LV_SphereSpotLightAttenuationLUT(float sqdist, float3 dirN, float sqlightSize, float3 color,
                                float spotMask, float cosAngle, uint customId, float occlusion, inout float3 L0,
                                inout float3 L1r, inout float3 L1g, inout float3 L1b, inout uint count)
            {
                                                                  ...
                                                                  inout float3 L1b, out float4 occlusion)
            {
                ...
                uint count = min(_UdonLightVolumeOcclusionCount == 0 ? additiveCount : volumesCount,
                           VRCLV_MAX_VOLUMES_COUNT);
                // Only use all volumes if occlusion volumes are enabled
                                      ...
                                      float3 L1r, float3 L1g, float3 L1b)
            {
                float3 specColor = max(float3(dot(reflect(-L1r, worldNormal), viewDir),
                                                  dot(reflect(-L1g, worldNormal), viewDir),
                                                  dot(reflect(-L1b, worldNormal), viewDir)), 0);
            ...
            float blendColorBurn(float base, float blend)
            {
                return (blend == 0.0)
                                               ? blend
                                               : max((1.0 - ((1.0 - base) *
                                          rcp(random_uniform_float_only_used_to_stop_compiler_warnings + blend))), 0.0);
            }
            ...
            float maskBlend(float baseMask, float blendMask, float blendType)
            {
                ...
                float w7 = step(7, t);
                float output = replace * w0 + subtract * w1 + multiply * w2 + divide * w3 + minVal * w4 + maxVal * w5 +
                    average * w6 + add * w7;
...
0, 0, 0, 1);
            ...
            float shouldPerformUDIMDiscard(float2 udim, float4 UDIMDiscardRows[4])
            {
                ...
                shouldDiscard *= any(float4(udim.y >= 0, udim.y < 4, udim.x >= 0, udim.x < 4));
            ...
            VertexOut vert(appdata v)
            {
                ...
                vertexAudioLink[0] = 0.0 == 0
                                                                      ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 0))[
                                                                          0]
                                                                      : AudioLinkData(
                                                                          ALPASS_FILTEREDAUDIOLINK + float2(
                                                                              (1 - 0.0) * 15.95, 0))[0];
                vertexAudioLink[1] = 0.0 == 0
                                                               ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 1))[
                                                                   0]
                                                               : AudioLinkData(
                                                                   ALPASS_FILTEREDAUDIOLINK + float2(
                                                                       (1 - 0.0) * 15.95, 1))[0];
                ...
                if (0.0 == 0) // Discard Vertices instead of just pixels
                {
                    ...
                    UDIMDiscardRows[0] = float4(0.0, 0.0, 0.0, 0.0);
                    UDIMDiscardRows[1] = float4(_UDIMDiscardRow1_0, _UDIMDiscardRow1_1, _UDIMDiscardRow1_2,
                              _UDIMDiscardRow1_3);
                    UDIMDiscardRows[2] = float4(_UDIMDiscardRow2_0, _UDIMDiscardRow2_1, _UDIMDiscardRow2_2,
                                 _UDIMDiscardRow2_3);
...
themeColorExposures.z), float4(1, 1, 1, 1).a);
                                    ...
                                    in PoiCam poiCam)
            {
                ...
                if (poiFragData.toggleVertexLights)
                {
                  ...
                  poiLight.vColor[index] * poiLight.vSaturatedDotNL[index] * poiLight.detailShadow); // Realistic
                    }
                    if (lightingMode == 1)
                    {
                        float2 ToonAddGradient = float2(0.0, 0.5);
            ...
            float2 decalUV(float uvNumber, float2 position, half rotation, half rotationSpeed, half2 scale,
                ...
                in float symmetryMode, in float mirroredUVMode,
            ...
            struct PoiDecal
            {
                   ...
                   in PoiCam poiCam)
                {
                    uv = decalUV(m_DecalTextureUV, m_DecalPosition, m_DecalRotation + decalRotation,
                        m_DecalRotationSpeed, decalScale, m_DecalSideOffset + sideMod,
                        m_DecalDepth, m_DecalSymmetryMode, m_DecalMirroredUVMode,
                        poiMesh, poiCam);
                    ...
                    decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb,
                             m_DecalHueShift + poiLight.nDotVCentered *
                    ...
                    in PoiMesh poiMesh, in PoiCam poiCam, float2 scaleMultiplier = float2(1, 1))
                {
                 ...
                 m_DecalDepth, m_DecalSymmetryMode, m_DecalMirroredUVMode, poiMesh, poiCam);
                    ...
                    decalColor.rgb = decalHueShift(m_DecalHueShiftEnabled, decalColor.rgb,
                      m_DecalHueShift + poiLight.nDotVCentered * m_DecalHueAngleStrength,
                      m_DecalHueShiftSpeed, m_DecalHueShiftColorSpace, m_DecalHueShiftSelectOrShift);
                    decalColor.a *= decalMask[m_DecalMaskChannel] * applyTilingClipping(m_DecalTiled, uv);
                }
                                                  ...
                                                  in PoiMesh poiMesh, in PoiCam poiCam)
                {
                    ...
                    float4 sample0 = tex2D(decalTexture,
                                           poiUV(uvSample0, m_DecalTexture_ST) + m_DecalTexturePan * _Time.x,
                                           dduvSample0.xy, dduvSample0.zw) * float4(
                        poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex), m_DecalColor.a);
                    float4 sample1 = tex2D(decalTexture,
                                           poiUV(uvSample1, m_DecalTexture_ST) + m_DecalTexturePan * _Time.x,
                           ...
                           in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
                {
                    ...
                    if (m_DecalOverrideAlpha && !faceMaskedOut)
                    {
                        ...
                        if (m_DecalGlobalMask > 0)
                        {
                            maskWeight = maskBlend(maskWeight, poiMods.globalMask[m_DecalGlobalMask - 1],
                                                   m_DecalGlobalMaskBlendType);
                        }
                        bool applyMask = (m_DecalOverrideAlphaMode == 2 || m_DecalOverrideAlphaMode == 3);
                        bool checkBounds = (m_DecalOverrideAlphaMode == 1 || m_DecalOverrideAlphaMode == 3) && !
                            m_DecalTiled;
                        if (!checkBounds || (uv.x > 0 && uv.x < 1 && uv.y > 0 && uv.y < 1))
                        {
                            float weight = applyMask ? maskWeight : 1.0;
                            if (m_DecalOverrideAlpha == 1) poiFragData.alpha = lerp(
                                poiFragData.alpha, finalAlpha, weight);
                            if (m_DecalOverrideAlpha == 2) poiFragData.alpha = lerp(
                                poiFragData.alpha, saturate(poiFragData.alpha * finalAlpha), weight);
                            if (m_DecalOverrideAlpha == 3) poiFragData.alpha = lerp(
                                poiFragData.alpha, saturate(poiFragData.alpha + finalAlpha), weight);
                            if (m_DecalOverrideAlpha == 4) poiFragData.alpha = lerp(
                                poiFragData.alpha, saturate(poiFragData.alpha - finalAlpha), weight);
                            if (m_DecalOverrideAlpha == 5) poiFragData.alpha = lerp(
                                poiFragData.alpha, min(poiFragData.alpha, finalAlpha), weight);
                            if (m_DecalOverrideAlpha == 6) poiFragData.alpha = lerp(
                                poiFragData.alpha, max(poiFragData.alpha, finalAlpha), weight);
                        }
                    }
                             ...
                             in PoiLight poiLight)
            {
                ...
                Decal0.m_DecalPosition = float4(0.2486265, 2.399017, 0, 0);
                ...
                {
                    udonVideoAspectRatio = Decal0.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, 0.0, 0.0);
                    if (0.0)
                    {
                ...
                Decal1.m_DecalRotation = 90.0;
                Decal1.m_DecalScale = float4(0.5, 0.5, 1, 0);
                Decal1.m_DecalSideOffset = float4(0, 0, 0, 0);
                Decal1.m_DecalPosition = float4(0.2494564, 1.719184, 0, 0);
        ...
        sin(((_Time.x + timeOffset) + rando.z * PI) * speed)) * Range;
            ...
            VertexOut vert(appdata v)
            {
                ...
                vertexAudioLink[0] = 0.0 == 0
                                            ...
                                            : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - 0.0) * 15.95, 0))[
                                                0];
                vertexAudioLink[1] = 0.0 == 0
        ...
        m_DecalColor.a);
                               ...
                               in PoiMesh poiMesh, in PoiCam poiCam)
                {
                    ...
                    float4 dduvSample1 = any(fwidth(uvSample1) > .5)
                                                                                 ? 0.001
                                                                                 : float4(
                                                                                     ddx(uvSample1) * m_DecalTexture_ST.
                                                                                     x, ddy(uvSample1) *
                                                                                     m_DecalTexture_ST.y);
                    float4 sample0 = tex2D(decalTexture,
                              poiUV(uvSample0, m_DecalTexture_ST) + m_DecalTexturePan * _Time.x, dduvSample0.xy,
                              dduvSample0.zw) * float4(poiThemeColor(poiMods, m_DecalColor.rgb, m_DecalColorThemeIndex),
                m_DecalColor.a);
                    float4 sample1 = tex2D(decalTexture,
